...
const KeyboardPage = () => {
  ...
  const pills = ["ALL", ...new Set(keyboards.map((item) => item.category))];

  const specsMap = [
    { icon: KeyboardIcon, key: "switches", label: t("keyboardPage.switches") },
    { icon: Layers, key: "layout", label: t("keyboardPage.layout") },
    { icon: Wifi, key: "connectivity", label: t("keyboardPage.connectivity") },
    { icon: Gauge, key: "keycaps", label: t("keyboardPage.keycaps") },
    { icon: Droplets, key: "lighting", label: t("keyboardPage.lighting") },
  ];

  const filteredKeyboards = useMemo(() => {
    if (filter === "ALL") return keyboards;
    return keyboards.filter((item) => item.category === filter);
  }, [filter]);
  ...
  const nextProduct = () => {
    if (selectedProductIndex !== null && selectedProductIndex < filteredKeyboards.length - 1) {
      const nextIndex = selectedProductIndex + 1;
      setSelectedProduct(filteredKeyboards[nextIndex]);
  ...
  const prevProduct = () => {
    if (selectedProductIndex !== null && selectedProductIndex > 0) {
      ...
      setSelectedProduct(filteredKeyboards[prevIndex]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 transition-colors duration-300 dark:bg-gray-900">
      <section className="container mx-auto px-4 py-6 transition-all duration-300">
        <div className="flex flex-wrap gap-2">
          {pills.map((pill) => (
            ...
            >
              {pill === "ALL" ? t("keyboardPage.all") : pill}
            </button>
          ))}
        </div>

        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredKeyboards.map((item, idx) => (
            <motion.article
              key={item.id}
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 pt-3 dark:border-gray-700">
                  <div className="flex items-center gap-1 text-[10px] text-gray-500 dark:text-gray-400">
                    ...
                    <span>{t("keyboardPage.deliveryTime")}</span>
                  </div>
                </div>
              </div>
            </motion.article>
      ...
      <ProductModal
        ...
        specsMap={specsMap}
        t={t}
        pageKey="keyboardPage"
        allProducts={filteredKeyboards}
